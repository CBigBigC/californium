...
import org.eclipse.californium.scandium.DtlsDynClusterConnector;
...
import org.eclipse.californium.scandium.config.DtlsClusterConfig.Builder;
...
public class ExtendedTestServer extends AbstractTestServer {
		...
		try {
			...
			if (config.cluster != null) {
				...
				if (config.cluster.k8sCluster != null) {
					k8sGroup = new K8sManagementClient(config.cluster.k8sCluster.externalPort);
			...
			if (k8sGroup != null) {
				server.addClusterEndpoint(secondaryExecutor, config.cluster.k8sCluster.dtls, k8sGroup.getNodeID(),
						config.cluster.k8sCluster.cluster, null, k8sGroup, config);
				...
				for (ClusterNode cluster : config.cluster.simpleCluster.dtlsClusterNodes) {
					if (cluster.dtls != null) {
						if (config.cluster.simpleCluster.dtlsClusterGroup != null) {
							group = new ClusterGroup(config.cluster.simpleCluster.dtlsClusterGroup,
									config.cluster.simpleCluster.dtlsClusterGroupSecurity);
						}
						server.addClusterEndpoint(secondaryExecutor, cluster.dtls, cluster.nodeId, cluster.cluster,
								nodes, group, config);
					}
				}
			}
	...
	private void addClusterEndpoint(ScheduledExecutorService secondaryExecutor, InetSocketAddress dtlsInterface,
			int nodeId, InetSocketAddress clusterInternalInterface, ClusterNodesProvider nodesProvider,
		...
		if (nodesDiscoverer != null) {
			builder.setConnector(
					new DtlsDynClusterConnector(dtlsConfigBuilder.build(), clusterInternalInterface, nodesDiscoverer));
		} else if (nodesProvider != null) {
			builder.setConnector(
					new DtlsClusterConnector(dtlsConfigBuilder.build(), clusterInternalInterface, nodesProvider));
		}
	...
	private static class ClusterGroup implements ClusterNodesDiscover {
		...
		private final DtlsClusterConfig config;

		private ClusterGroup(List<InetSocketAddress> group, String secret) {
			Builder builder = DtlsClusterConfig.builder();
			if (secret != null) {
				SecretKey key = SecretUtil.create(secret.getBytes(), "PSK");
				builder.setSecure("dtls-mgmt", key);
		...
		@Override
		public DtlsClusterConfig getConfiguration() {
			return config;
		}
